...
## Visualizing with the jitter plot
```{r}
...
post_meal_data |>
  mutate(
    percent_body_fat = (Fat.mass.DXA / Weight) * 100,
    mutate = if_else( Age < 40, "young", "old")
  ) |>
...
tidied_post_meal_data |>
  group_by(Group) |>
  summarise("Mean Age (years)" = round(mean(Age), 1),
...
post_meal_data |>
  ...
  summarise(
    "median AUC c-peptide" = round(median(auc_cp), 1),
     "median AUC plasma glucose" = round(median(auc_pg), 1),
     "median auc plasma insulin" = round(median(auc_ins), 1),
  ) |>
  # Task 5.
  rename(
    "Age group" = age_group,
